...
func RetrieveDevice(id primitive.ObjectID) (lib.IDevice, lib.IStatus) {
	...
	err := lib.MyDB.Collection(DeviceCollectionName).FindOne(context.TODO(), bson.M{"resource.id": id}).Decode(&deviceToRetrieve)
	if err != nil {
...
func AddDevice(device lib.IDevice) (primitive.ObjectID, lib.IStatus) {
	...
	_, err := lib.MyDB.Collection(DeviceCollectionName).InsertOne(context.TODO(), device)
	if err != nil {
...
func UpdateDevice(device lib.IDevice) (primitive.ObjectID, lib.IStatus) {
	...
	_, err := lib.MyDB.Collection(DeviceCollectionName).UpdateOne(context.TODO(),bson.M{"resource.id": device.Resource.ID}, device)
	if err != nil {
...
func RetrieveAllDevices() ([]lib.IDevice, lib.IStatus) {
	...
	cursor,_ := lib.MyDB.Collection(DeviceCollectionName).Find(context.TODO(), bson.M{})
...
func AddDeviceData(deviceData lib.IDeviceData) (primitive.ObjectID, lib.IStatus) {
	...
	_, err := lib.MyDB.Collection(temperatureCollectionName).InsertOne(context.TODO(), deviceData)
	if err != nil {
...
func RemoveDevice(id primitive.ObjectID) (primitive.ObjectID, lib.IStatus){
	_, err := lib.MyDB.Collection(DeviceCollectionName).DeleteOne(context.TODO(), bson.M{"resource.id": id})
	if err != nil {
...
func RetrieveDeviceData(id primitive.ObjectID, startDate time.Time, endDate time.Time) ([]lib.IDeviceData, lib.IStatus) {
	...
	cursor, _ := lib.MyDB.Collection(temperatureCollectionName).Find(context.TODO(), bson.M{"deviceid": id, "time": bson.M{"$lte": endDate, "$gte": startDate}})
